...
RECOMMENDED VERSION
VERSION 1.0.0
Copyright thelastpenguinв„ў 
	All rights are reserved.
	Neither this script nor any edit of this script or partial code segment taken from this script
	may be used in any form without explicit permission from it's creator thelastpenguinв„ў 
...
do
	...
	local xfn_true = xfn.fn_const( true );
	local xfn_false = xfn.fn_const( false );
	function _R.Panel:DoCursor( vOrigin, cursorNormal )
		...
		if self.hoveredPanel ~= hovered then
			...
			if ValidPanel( self.hoveredPanel ) then
				...
				(self.hoveredPanel.OnCursorExited or xfn.nothing)( self.hoveredPanel );
...
hook.Add('PostDrawTranslucentRenderables', 'ba-3d2d-p', function()
	...
	userpanels = xfn.filter( userpanels, ValidPanel );
...
concommand.Add( 'testmenu', function()
	if ValidPanel( panel ) then panel:Remove( ) return end
	
	panel = vgui.Create( 'DPanel' );
	panel.CalcLocation = vgui.CalcParentedOffset( Vector( 11, 0, 0 ), (function()
		local ang = LocalPlayer():EyeAngles();
		return Angle( 0, ang.y, ang.r );
	end)(), 0.02 );
	panel:SetSize( 500, 600 );
	
	function panel:Paint( w, h )
		surface.SetDrawColor(20,20,20,245);
		surface.DrawRect(0,0,w,h);
		surface.SetDrawColor(255,255,255);
		surface.DrawOutlinedRect(0,0,w,h);
	end
	
	local title = Label( 'Job Menu', panel );
	title:SetFont('darkrp_20');
	title:SetPos( 10, 0 );
	title:SizeToContents( );
	
	local main = vgui.Create( 'DPanel', panel );
	main.Paint = xfn.nothing;
	main:SetSize( panel:GetWide(), panel:GetTall()-title:GetTall());
	main:SetPos( 0, title:GetTall() );
	
	local sidepanel ;
	local sidepanel_calcpos = vgui.CalcParentedOffset( Vector( 11, 0, 0 ), (function()
		local ang = LocalPlayer():EyeAngles();
		return Angle( 0, ang.y - 45, ang.r );
	end)(), 0.02 );
	
	function panel:OnRemove( )
		if ValidPanel( sidepanel ) then sidepanel:Remove( ); end
	end
